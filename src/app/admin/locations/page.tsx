...
export default function AdminLocationsPage() {
  ...
  const handleEdit = (location: LocationRecord) => {
    ...
    setForm({
      name: location.name,
      type: location.type,
      campus: location.campus ?? campusOptions[0] ?? "Main Campus",
    ...
    <section className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-[#64615C]">
            View, create, edit, and delete campus locations.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-xl border border-[#EAEAEA] bg-white p-4 ">
        <input
          ...
          placeholder="Search locations by name, type, description, hours, accessibility, email, or phone"
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-2">
        {filteredLocations.map((location) => (
          ...
          >
            ...
            <p className="mt-2 text-sm text-[#787774]">{location.type}</p>
            <p className="mt-3 text-[#64615C]">{location.description}</p>
          ...
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-[#EAEAEA] bg-white p-6 shadow-[0_20px_60px_-40px_rgba(17,17,17,0.22)]">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-[#64615C]">
                  Fill in the details below and save the location.
                </p>
              </div>
              <button
                type="button"
            ...
            <form noValidate className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
              ...
              <select
                value={form.campus}
                onChange={(e) =>
                  setForm((current) => ({ ...current, campus: e.target.value }))
                }
                className="rounded-xl border border-[#D8D6D0] px-4 py-3"
                required
              >
