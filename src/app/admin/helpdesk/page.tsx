...
export default function AdminHelpdeskPage() {
  ...
  const handleUpdate = async (id: number, updates: Partial<Pick<HelpdeskTicketRecord, "status" | "assigned_to" | "admin_notes">>) => {
    const supabase = getSupabaseClient();
    if (!supabase) return;
    const payload = {
      ...updates,
  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-600">Review, assign, update, and delete support requests.</p>
      </div>
          ...
          <article key={ticket.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <select defaultValue={ticket.status} onChange={(event) => void handleUpdate(ticket.id, { status: event.target.value as HelpdeskTicketRecord["status"] })} className="rounded-xl border border-slate-300 px-4 py-3 text-slate-900">
                  {statusOptions.map((status) => (<option key={status} value={status}>{status}</option>))}
                </select>

                <select defaultValue={ticket.assigned_to ?? ""} onChange={(event) => void handleUpdate(ticket.id, { assigned_to: event.target.value || null })} className="rounded-xl border border-slate-300 px-4 py-3 text-slate-900">
                  <option value="">Unassigned</option>
                  {profiles.filter((userProfile) => userProfile.role === "admin" || userProfile.role === "teacher").map((userProfile) => (<option key={userProfile.id} value={userProfile.id}>{userProfile.full_name}</option>))}
                </select>

                <input defaultValue={ticket.admin_notes ?? ""} onBlur={(event) => void handleUpdate(ticket.id, { admin_notes: event.target.value })} placeholder="Admin notes" className="rounded-xl border border-slate-300 px-4 py-3 text-slate-900" />
              </div>
            </div>
          </article>
        ))}
