  ...
  return (
    <section className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Manage Societies</h1>
          <p className="mt-2 text-slate-600">View, create, edit, and delete societies.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {societies.map((society) => (
          <article key={society.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-xl font-semibold text-slate-900">{society.name}</h2>
            <p className="mt-2 text-sm text-slate-500">{society.category}</p>
            <p className="mt-3 text-slate-600">{society.description}</p>
            <div className="mt-5 flex gap-3">
              <button type="button" onClick={() => handleEdit(society)} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-900">Edit</button>
              <button type="button" onClick={() => void handleDelete(society.id)} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-medium text-white">Delete</button>
            </div>
          </article>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">{editingId ? "Edit Society" : "Add Society"}</h2>
                <p className="mt-1 text-sm text-slate-600">Fill in the details below and save the society.</p>
              </div>
            ...
            <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
              <input value={form.name} onChange={(e) => setForm((current) => ({ ...current, name: e.target.value }))} placeholder="Society name" className="rounded-xl border border-slate-300 px-4 py-3" required />
              ...
              <input value={form.meeting_day} onChange={(e) => setForm((current) => ({ ...current, meeting_day: e.target.value }))} placeholder="Meeting day" className="rounded-xl border border-slate-300 px-4 py-3" />
              ...
              <div className="flex gap-3 md:col-span-2">
                <button type="submit" className="rounded-xl bg-slate-900 px-5 py-3 text-white">{editingId ? "Update society" : "Create society"}</button>
