...
import { getUserSocietyMemberships } from "@/lib/data";
...
export default function AccountPage() {
  ...
  useEffect(() => {
    const loadMemberships = async () => {
      ...
      const memberships = await getUserSocietyMemberships(user.id);
      setSelectedSocietyIds(
        memberships.length ? memberships.map((item) => String(item.society_id)) : [""]
      );
    };
