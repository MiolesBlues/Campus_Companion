...
function eventRecommendationScore(event: EventWithTags, campus: string | null | undefined, interests: string[] | null | undefined, preferredCategories: string[] | null | undefined) {
  let score = 0;
...
export default function Home() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [events, societies] = await Promise.all([getEvents(), getSocietiesList()]);
      setUpcomingEvents(events.slice(0, 3));

      const rankedEvents = [...events]
        .sort((a, b) => eventRecommendationScore(b, profile?.campus, profile?.interests, profile?.preferred_event_categories) - eventRecommendationScore(a, profile?.campus, profile?.interests, profile?.preferred_event_categories))
        .filter((event) => eventRecommendationScore(event, profile?.campus, profile?.interests, profile?.preferred_event_categories) > 0)
        .slice(0, 3);
      setRecommendedEvents(rankedEvents);

      const rankedSocieties = [...societies]
        .sort((a, b) => societyRecommendationScore(b, profile?.interests, profile?.preferred_society_categories) - societyRecommendationScore(a, profile?.interests, profile?.preferred_society_categories))
        .filter((society) => societyRecommendationScore(society, profile?.interests, profile?.preferred_society_categories) > 0)
        .slice(0, 3);
      setRecommendedSocieties(rankedSocieties);
    };

    void loadData();
  }, [profile?.campus, profile?.interests, profile?.preferred_event_categories, profile?.preferred_society_categories]);

  const hasRecommendations = useMemo(() => recommendedEvents.length > 0 || recommendedSocieties.length > 0, [recommendedEvents.length, recommendedSocieties.length]);

  return (
    <section className="space-y-6 sm:space-y-8">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <span className="inline-flex rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700">Student Life Dashboard</span>
        <h1 className="mt-4 text-3xl font-bold text-slate-900 sm:text-4xl">Campus Companion</h1>
        <p className="mt-3 max-w-2xl text-base text-slate-600 sm:text-lg">Your closed campus platform for events, timetables, societies, support, and day-to-day student life.</p>
      ...
      {hasRecommendations && <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-semibold text-slate-900 sm:text-2xl">Recommended for you</h2><p className="mt-1 text-sm text-slate-600">Based on your campus, interests, and preferences.</p></div><Link href="/account" className="text-sm font-medium text-blue-700 hover:text-blue-900">Update preferences</Link></div><div className="mt-6 grid gap-6 lg:grid-cols-2"><div><h3 className="text-lg font-semibold text-slate-900">Events</h3><div className="mt-3 space-y-3">{recommendedEvents.length > 0 ? recommendedEvents.map((event) => (<article key={event.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700">{event.category}</span><span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-medium text-slate-700">Recommended</span></div><h4 className="mt-3 font-semibold text-slate-900">{event.title}</h4><p className="mt-1 text-sm text-slate-600">{event.event_date} • {event.location}</p></article>)) : <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600">No event recommendations yet.</div>}</div></div><div><h3 className="text-lg font-semibold text-slate-900">Societies</h3><div className="mt-3 space-y-3">{recommendedSocieties.length > 0 ? recommendedSocieties.map((society) => (<article key={society.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">{society.category}</span><span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-medium text-slate-700">Suggested</span></div><h4 className="mt-3 font-semibold text-slate-900">{society.name}</h4><p className="mt-2 text-sm text-slate-600">{society.description}</p></article>)) : <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600">No society recommendations yet.</div>}</div></div></div></div>}
